...
describe("Nodejs crypto test suite", () => {
  test("Sign and verify nodejs P256", () => {
    ...
    expect(cryptokit.P256.verify(messageToSignWithP256, messageP256Signature, P256PublicKey)).toBe(true);
  });
  ...
  test("Sign and verify nodejs Ed25519", () => {
    ...
    expect(cryptokit.Ed25519.verify(messageToSignWithEd25519, messageEd25519Signature, Ed25519PublicKey)).toBe(true);
  });
  ...
  test("Encrypt e2e with iOS P256", () => {
    ...
    expect(cryptokit.P256.encrypt(messageToEncryptWithP256, P256PrivateKey, iOSP256PublicKeyObject).message).toEqual(
      expect.any(String)
    );
  });
  ...
  test("Encrypt e2e with iOS X25519", () => {
    const messageToEncryptWithX25519 =
      "Hi! I am an end-to-end encrypted example message from nodejs with symmetric P256 key";
    expect(
      cryptokit.X25519.encrypt(messageToEncryptWithX25519, X25519PrivateKey, iOSX25519PublicKeyObject).message
